...
#ifdef NETWORK
    char buffer[256];
    sprintf(buffer, "MISSILE %d %d %lf %lf %d", (int) xpos, (int) ypos, xvel, yvel, weaponType);
...
int MISSILE::applyPhysics()
{
    ...
    if (!physics)
    {
        ...
        if ( (!detonate) && ((type == SMALL_MIRV) || (type == CLUSTER_MIRV)) )
        {
            ...
            if ((above_ground > 0) && (above_ground < TIGGER_HEIGHT) && (yv > 0))
                detonate = 1;
        }
        ...
        {
            hitSomething = 0;
            if ( (y >= _global->screenHeight) && ((_env->current_wallType == WALL_STEEL)
                 || (_env->current_wallType == WALL_WRAP)) )
            {
                detonate = TRUE;
                hitSomething = 1;
    ...
    else
    {
        if (type >= SML_ROLLER && type <= DTH_ROLLER)
        {
            ...
            if ((x < 2) ||(x > (_global->screenWidth - 3)) ||(y > (_global->screenHeight - 3))
...
void MISSILE::trigger()
{
    ...
    if ((_env->current_wallType == WALL_WRAP) && (type < WEAPONS))
    {
        ...
        {
            if   ((type >= TREMOR && type <= TECTONIC) || (type >= RIOT_CHARGE && type <= RIOT_BLAST))
        ...
        else
        {
            ...
            if (type == NAPALM_JELLY)
            {
                if (bIsGrowing)
                {
                    ...
                    setUpdateArea((int) x - (iGrowRadius + 1), (int) y - (iGrowRadius + 1),
                                  (iGrowRadius + 1) * 2, (iGrowRadius + 1) * 2);
                }
                else
                {
