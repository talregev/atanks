...
int Parse_Client_Data(GLOBALDATA *global, ENVIRONMENT *env, char *buffer)
{
    ...
    else if (!strcmp(args[0], "SCREEN"))
    {
        ...
        if ((width == global->screenWidth) && (height == global->screenHeight))
    ...
    else if (!strcmp(args[0], "TELEPORT"))
    {
        ...
        if ((player_num >= 0) && (player_num < global->numPlayers) && (global->players[player_num]->tank))
            new TELEPORT(global, env, global->players[player_num]->tank, new_x, new_y,
                         TANKHEIGHT * 4 + GUNLENGTH, 120);
    ...
    else if (!strcmp(args[0], "WEAPON"))
    {
        ...
        sscanf(args[2], "%d", &amount);
        if ((weapon_index >= 0) && (weapon_index < WEAPONS) && (amount >= 0) && (amount <= 99))
