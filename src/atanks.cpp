    ...
    while (1)
    {
        ...
        while (get_cclock() > 0 || skippingComputerPlay)
        {
            ...
            if (skippingComputerPlay)
            {
                ...
                if (AI_clock > MAX_AI_TIME)
                {
                    ...
                    {
                        if ((global->players[player_index]) && (global->players[player_index]->tank))
            ...
            if (tanksfall && (env->stage != STAGE_ENDGAME))
            {
                for (objCount = 0; (ltank = (TANK*) env->getNextOfClass(TANK_CLASS, &objCount)) && ltank; count--, objCount++)
                {
                    ...
                    if ((ltank->l <= 0) && !anyExploding)
                    {
                        ...
                        if (ltank->destroy)
                        {
                            ...
                            if (!humanPlayers)
                            {
                                ...
                                if ((iMostIntelligentPlayer < (int) DEADLY_PLAYER) && (iMostIntelligentPlayer >= (int) USELESS_PLAYER) && (iNumPlayers > 1))
                                    for (int i = 0; i < global->numPlayers; i++)
                                    {
                                        if (global->players[i]->tank)
                                        {
                    ...
                    if ((global->max_fire_time > 0.0) && ltank && (ltank->player->type == HUMAN_PLAYER) && (!env->stage))
                    {
                        if (global->Check_Time_Changed())
                        {
                            int ran_out_of_time;
            ...
            else if (global->computerPlayersOnly && ((int) global->skipComputerPlay >= SKIP_HUMANS_DEAD))
            {
        ...
        if (env->stage == STAGE_ENDGAME)
        {
            ...
            if (roundEndCount >= WAIT_AT_END_OF_ROUND)
            {
                ...
                {
                    if ((global->players[tank_index]->tank) && (global->players[tank_index]->tank->l > 0))
                        alive = true;
