...
void FLOATTEXT::draw(BITMAP *dest)
{
    ...
    if (_current.w)
    {
        ...
        if ((_env->dShadowedText > 0.0) || (_env->dFadingText > 0.0))
        {
            ...
            {
            case LEFT:
                iBackCol = _env->getAvgBgColor(_current.x, _current.y, _current.x + _current.w,
                                               _current.y + _current.h, xv, yv);
                break;
            case RIGHT:
                iBackCol = _env->getAvgBgColor(_current.x - _current.w, _current.y - _current.h,
                                               _current.x, _current.y, xv, yv);
                break;
            case CENTRE:
        ...
        {
            if (_env->dShadowedText > 0.0)
                textout_ex(dest, font, _text, _current.x + 1, _current.y + 1, iShadCol, -1);
            textout_ex(dest, font, _text, _current.x, _current.y, iFrontCol, -1);
        }
        else if (_align == RIGHT)
        {
            if (_env->dShadowedText > 0.0)
                textout_ex(dest, font, _text, _current.x - _current.w + 1, _current.y - _current.h + 1, iShadCol, -1);
            textout_ex(dest, font, _text, _current.x - _current.w, _current.y - _current.h, iFrontCol, -1);
        }
        else
        {
            if (_env->dShadowedText > 0.0)
                textout_centre_ex(dest, font, _text, _current.x + 1, _current.y + 1, iShadCol, -1);
            textout_centre_ex(dest, font, _text, _current.x, _current.y, iFrontCol, -1);
        }
    }
}
