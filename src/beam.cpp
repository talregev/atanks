...
void tracePath(int *targetX, int *targetY, GLOBALDATA *global, ENVIRONMENT *env,
               int minRange, double xpos, double ypos, int angle)
{
    ...
    while ( (!hitSomething) && tx > -10 && tx < global->screenWidth + 10 && ty > -10 &&
           ty < global->screenHeight && (getpixel(env->terrain, (int)tx, (int)ty) == PINK) )
    {
        TANK *ltank;
        for (int objCount = 0; (ltank = (TANK*)env->getNextOfClass(TANK_CLASS, &objCount)) && ltank; objCount++)
        {
            if ( range > minRange && tx > ltank->x - TANKWIDTH && tx < ltank->x + TANKWIDTH
                 && ty > ltank->y && ty < ltank->y + TANKHEIGHT && ltank->l > 0 )
            {
                hitSomething = 1;
                ltank->requireUpdate();
            }
...
BEAM::BEAM(GLOBALDATA *global, ENVIRONMENT *env, double xpos, double ypos,
           int fireAngle, int weaponType) : PHYSICAL_OBJECT(),weap(NULL),points(NULL)
{
    player = NULL;
    drag = 0.00;
    mass = 0;
    ...
    if ( type >= SML_LIGHTNING && type <= LRG_LIGHTNING )
    {
        ...
        numPoints = 4 + rand() % 10;
        color = WHITE;
        play_sample((SAMPLE *) _global->sounds[LIGHTNING_SOUND],
                    env->scaleVolume(128 + (radius * 10)), 128, 500 + (radius * 50), 0);
    }
    else
    {
        numPoints = 2;
...
int BEAM::applyPhysics()
{
    ...
    {
        if (targetX > ltank->x - TANKWIDTH - radius && targetX < ltank->x + TANKWIDTH + radius
            && targetY > ltank->y - radius && targetY < ltank->y + TANKHEIGHT + radius && ltank->l > 0)
        {
            // hitSomething = 1;
            ltank->requireUpdate();
...
void BEAM::draw(BITMAP *dest)
{
    ...
    if (type >= SML_LIGHTNING && type <= LRG_LIGHTNING)
    {
        ...
        for (int point = 1; point < numPoints; point++)
        {
            ...
            addUpdateArea(MIN(x1,x2) - radius * 6, MIN(y1,y2) - radius * 6,
                          MAX(x1,x2) + radius * 2 * 6, MAX(y1,y2) + radius * 2 * 6);
        }
    }
    else if (type >= SML_LAZER && type <= LRG_LAZER)
    {
        ...
        for (int point = 1; point < numPoints; point++)
        {
            ...
            addUpdateArea(MIN(x1,x2) - radius * 2, MIN(y1,y2) - radius * 2,
                          MAX(x1,x2) + radius * 2 * 2, MAX(y1,y2) + radius * 2 * 2);
        }
    }
