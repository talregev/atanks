...
void SATELLITE::Draw(BITMAP *dest)
{
    ...
    draw_sprite(dest, (BITMAP *) _global->misc[SATELLITE_IMAGE], x, y);
...
void SATELLITE::Shoot()
{
    ...
    if (!chance)    // !% chance to fire
    {
        ...
        if (_env->satellite == LASER_WEAK)
            laser_type = SML_LAZER;
        else if (_env->satellite == LASER_STRONG)
            laser_type = MED_LAZER;
        else if (_env->satellite == LASER_SUPER)
            laser_type = LRG_LAZER;
        else
            return;

        int angle = rand() % 30;
        BEAM *my_beam = new BEAM(_global, _env, (xv < 0) ? x + 10: x + 40, y + 20, angle, laser_type);
        if (!my_beam)
            return;
        my_beam->player = NULL;
